...
def load_feature_geometry(dir: str) -> dict:
    ...
    feature_geometry.loc[:, 'tier'] = feature_geometry['path'].apply(lambda x: len(x.split(' | ')))
    feature_geometry.loc[:, 'parent'] = feature_geometry['path'].apply(lambda x: x.split(' | ')[-1])
    feature_geometry.loc[:, 'n_sisters'] = feature_geometry['parent'].apply(
        lambda x: feature_geometry['parent'].to_list().count(x)
    )
    feature_geometry.loc[:, 'n_descendants'] = feature_geometry['feature'].apply(
        lambda x: len([
    ...
    feature_geometry.loc[:, 'n_distinctions'] = (feature_geometry['n_sisters'] + 1) + (feature_geometry['n_descendants'])
    weights = np.array([
